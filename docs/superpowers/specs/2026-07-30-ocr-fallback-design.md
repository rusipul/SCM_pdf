...
> 최종적으로 OCR 기능은 도입하지 않고, 텍스트 레이어 감지(기존 방식)만 사용하는 버전으로 유지하기로 결정함. 사유: 실제 OCR 테스트에서 건별 합계 라벨("합계Total") 인식이 크게 불안정함이 확인되어(아래 "실제 진단 결과" 참고) 정확도 리스크가 있었고, 텍스트 전용 처리로도 충분하다고 판단함. 이 설계 문서와 구현 계획(`docs/superpowers/plans/2026-07-30-ocr-fallback.md`)은 실제로 구현되지 않았으며, 참고용으로만 남겨둠.
